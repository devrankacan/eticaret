          ...
          {mainCats.map(cat => (
            <div key={cat.id}>
              ...
              {subCats.filter(s => s.parentId === cat.id).map(sub => (
                <div key={sub.id} className="bg-gray-50 rounded-xl p-3 ml-8 flex items-center gap-3 mt-1.5">
        ...
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-lg">
            ...
            <div className="p-5 space-y-4">
              ...
              <div>
                <label className="text-sm font-medium text-gray-700 mb-1 block">Görsel URL</label>
                <input className="w-full border border-gray-200 rounded-xl px-3 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-primary-300" placeholder="https://..." value={form.image} onChange={e => setForm(f => ({ ...f, image: e.target.value }))} />
              </div>
              <div>
